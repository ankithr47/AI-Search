...
def OX_crossover(A,B):
    ...
    split1 = random.randint(0, len(A) - 2)
    split2 = random.randint(split1 + 1, len(A) - 1)
    A_mid = A[split1: split2]
    A_mid_set = set(A_mid)
    B1 = B[split2:]
    B2 = B[:split2]
    remaining = B1 + B2
    remaining_filtered = [city for city in remaining if city not in A_mid_set]
    after_len = len(A) - split2
    seg2 = remaining_filtered[:after_len]
